...
- **Data-infra case-study images:** two case studies have no product UI to screenshot, so they use honest stand-ins instead of fake screenshots: the eviction workstream shows the **North Texas Evictions** dashboard (its public face; `north-texas-evictions.png`, shared with the NTE tool card), and **Building CPAL's Data Function** shows a **rendered architecture diagram** (`public/images/projects/cpal-data-function.png`, a 1280×720 canyon-palette flow: sources → Databricks platform → data products → people, generated via Playwright + inline HTML/SVG).
