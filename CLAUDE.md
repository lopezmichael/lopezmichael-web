...
- [x] ~~Add resume PDF + flip `hasResumePdf`~~ DONE — `public/files/Michael_Lopez_Resume.pdf` (clean single-column ATS resume rendered from `resume.ts` data via Playwright `page.pdf()` with embedded Inter; 2 pages). `hasResumePdf = true`. NOTE: it's a point-in-time render — regenerate if `resume.ts` changes (the generator was a one-off; data was inlined, so it does not auto-update).
